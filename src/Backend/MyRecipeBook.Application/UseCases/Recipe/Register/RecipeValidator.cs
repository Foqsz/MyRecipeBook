...
public class RecipeValidator : AbstractValidator<RequestRecipeJson>
{
    public RecipeValidator()
    {
        ...
        {
            instructionRule.RuleFor(instruction => instruction.Step).GreaterThan(0).WithMessage(ResourceMessagesException.NON_NEGATIVE_AND_NON_EMPTY_INSTRUCTION_STEP);
            instructionRule
        ...
        RuleFor(recipe => recipe.Instructions)
            .Must(instruction => instruction.Select(i => i.Step).Distinct().Count() == instruction.Count) //must retorna um booleano, se for verdadeiro segue o codigo, se for falso retorna a messagem
